...
export class LivroUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.livroService.readById(Number.parseInt(id)).subscribe((livros) => {
        this.livros = livros;
      });
    }
  ...
  updateLivro(): void {
    ...
    this.livroService
      ...
      .pipe(
        catchError((error) => {
          this.livroService.showMessage(
            `Erro ao tentar alterar livro: ${error.message}`
          );
          return throwError(
            () => new Error(`Erro ao tentar alterar livro: ${error.message}`)
          );
        })
      )
      .subscribe({
        next: () => {
          this.livroService.showMessage("Livro alterado com sucesso!");

...
const postLivro = async (livro) => {
    ...
    const sql = "INSERT INTO livros(titulo, autor_id_fk, editora_id_fk, ano_publicado) VALUES ($1, $2, $3, $4)";
    const values = [livro.titulo, livro.autor_id_fk, livro.editora_id_fk, livro.ano_publicado];
    await insert_livro.query(sql, values);
}

const patchLivro = async (id, livro) => {
    const update_livro = await connection.connect();
    const sql = "UPDATE livros SET titulo=$1, autor_id_fk=$2, editora_id_fk=$3, ano_publicado=$4 WHERE livro_id=$3";
    const values = [livro.titulo, livro.autor_id_fk, livro.editora_id_fk, livro.ano_publicado];
